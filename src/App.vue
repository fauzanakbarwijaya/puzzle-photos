    ...
    img.onload = () => {
        for (let row = 0; row < gridSize.value; row++) {
            for (let col = 0; col < gridSize.value; col++) {
                ...
                puzzlePieces.value.push({ id: row * gridSize.value + col, imgSrc: canvas.toDataURL() });
            }
        }
        shufflePuzzle();
    };
...
const onDrop = (event, dropIndex) => {
    ...
    if (!isNaN(dragIndex)) {
        [shuffledPieces.value[dragIndex], shuffledPieces.value[dropIndex]] = [shuffledPieces.value[dropIndex], shuffledPieces.value[dragIndex]];
        checkWin();
    }
};
  ...
  <div class="container vh-100 d-flex justify-content-center align-items-center text-center">
      <div class="card p-4 bg-dark text-light shadow-lg">
          ...
          <div v-if="imageUrl" class="mb-3">
              <img :src="imageUrl" alt="Uploaded Image" class="img-fluid rounded" style="max-width: 300px; border: 2px solid white;" />
          </div>
          ...
          <button @click="createPuzzle" class="btn btn-outline-primary mt-3">Mulai</button>

          <div class="mt-4 text-light" v-if="gameStarted">Waktu tersisa: {{ Math.floor(timer / 60) }}:{{ (timer % 60).toString().padStart(2, '0') }}</div>
          
          <div class="mt-4 d-flex flex-wrap" :style="{ width: canvasSize + 'px', height: canvasSize + 'px' }" v-if="gameStarted">
              <div v-for="(piece, index) in shuffledPieces" :key="piece.id" 
                   class="puzzle-piece" 
                   draggable="true"
                   @dragstart="onDragStart($event, index)"
                   @dragover="onDragOver"
                   @drop="onDrop($event, index)"
                   :style="{ width: pieceSize + 'px', height: pieceSize + 'px', backgroundImage: `url(${piece.imgSrc})`, backgroundSize: 'cover' }">
              </div>
          </div>
      </div>
  </div>
